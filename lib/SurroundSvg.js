...
var SurroundSvg = /*#__PURE__*/function (_Component) {
  ...
  function SurroundSvg() {
    ...
    _defineProperty(_assertThisInitialized(_this), "getPath", function (cur) {
      ...
      var r = _this.props.radius; // 大小弧判断

      var isLargeArc = cur.percent > 50 ? 1 : 0;
      var startX = cur.start.x * r + x0;
      var startY = cur.start.y * r + y0;
      var endX = cur.end.x * r + x0;
      var endY = cur.end.y * r + y0;
      var path = "M ".concat(startX, " ").concat(startY, " A  ").concat(r, " ").concat(r, " 0 ").concat(isLargeArc, " 1 ").concat(endX, " ").concat(endY);
      return {
        d: path,
        startX: startX,
        startY: startY,
  ...
  _createClass(SurroundSvg, [{
    ...
    get: function get() {
      ...
      data.forEach(function (item, index) {
        ...
        var prevArcObj = arcList[index - 1] || {};
        var percentAddPrev = index !== 0 ? prevArcObj.percentAddPrev + percent : percent;
        var deg = percentAddPrev / 100 * Math.PI * 2;
        var arcObj = {
          percent: percent,
          percentAddPrev: percentAddPrev,
          deg: deg,
          start: index === 0 ? {
            x: Math.cos(0),
            y: Math.sin(0)
        ...
        arcList.push(_objectSpread({}, arcObj, {
          source: item
        }));
      });
      return arcList;
